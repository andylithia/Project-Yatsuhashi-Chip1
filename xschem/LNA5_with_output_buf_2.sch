v {xschem version=3.0.0 file_version=1.2 }
G {}
K {}
V {}
S {}
E {}
N -560 -530 -560 -470 {
lab=Vref}
N -560 -500 -510 -500 {
lab=Vref}
N -510 -500 -510 -440 {
lab=Vref}
N -710 -1000 -710 -980 {
lab=#net1}
N -710 -720 -710 -700 {
lab=#net2}
N -710 -910 -600 -910 {
lab=VDDI}
N -630 -970 -630 -910 {
lab=VDDI}
N -630 -1060 -630 -1030 {
lab=VDD}
N -800 -1060 -630 -1060 {
lab=VDD}
N -710 -920 -710 -910 {
lab=VDDI}
N -800 -640 -710 -640 {
lab=GND}
N -710 -790 -710 -780 {
lab=GNDI}
N -710 -790 -600 -790 {
lab=GNDI}
N -630 -790 -630 -740 {
lab=GNDI}
N -710 -640 -630 -640 {
lab=GND}
N -800 -800 -800 -640 {
lab=GND}
N -600 -910 -580 -910 {
lab=VDDI}
N -600 -790 -580 -790 {
lab=GNDI}
N -630 -680 -630 -640 {
lab=GND}
N -580 -910 -570 -910 {
lab=VDDI}
N -570 -910 -440 -910 {
lab=VDDI}
N -580 -790 -540 -790 {
lab=GNDI}
N -540 -790 -400 -790 {
lab=GNDI}
N -440 -910 -400 -910 {
lab=VDDI}
N -800 -1060 -800 -860 {
lab=VDD}
N -270 -300 -250 -300 {
lab=#net3}
N -815 -290 -815 -260 {
lab=#net4}
N -815 -300 -815 -290 {
lab=#net4}
N -210 -300 -190 -300 {
lab=GNDI}
N -170 -240 -170 -170 {
lab=GNDI}
N -190 -300 -170 -300 {
lab=GNDI}
N -170 -300 -170 -240 {
lab=GNDI}
N -815 -300 -805 -300 {
lab=#net4}
N -330 -600 -330 -570 {
lab=#net5}
N -330 -600 -200 -600 {
lab=#net5}
N -270 -570 -250 -570 {
lab=VDDI}
N -310 -570 -270 -570 {
lab=VDDI}
N -310 -720 -310 -570 {
lab=VDDI}
N -200 -600 -160 -600 {
lab=#net5}
N -590 -440 -590 -380 {
lab=GNDI}
N -590 -440 -560 -440 {
lab=GNDI}
N -560 -120 -490 -120 {
lab=GNDI}
N -330 -350 -330 -300 {
lab=#net6}
N -745 -300 -720 -300 {
lab=#net7}
N -660 -300 -440 -300 {
lab=#net6}
N -210 -340 -210 -330 {
lab=#net8}
N -490 -120 -280 -120 {
lab=GNDI}
N -210 -630 -210 -600 {
lab=#net5}
N -280 -635 -280 -600 {
lab=#net5}
N -280 -720 -280 -695 {
lab=VDDI}
N -210 -720 -210 -690 {
lab=VDDI}
N -20 -600 110 -600 {
lab=#net9}
N 40 -570 60 -570 {
lab=VDDI}
N 100 -570 120 -570 {
lab=GNDI}
N 0 -570 40 -570 {
lab=VDDI}
N 0 -720 0 -570 {
lab=VDDI}
N 110 -600 150 -600 {
lab=#net9}
N 120 -570 120 -510 {
lab=GNDI}
N 100 -630 100 -600 {
lab=#net9}
N 30 -635 30 -600 {
lab=#net9}
N 30 -720 30 -695 {
lab=VDDI}
N 100 -720 100 -690 {
lab=VDDI}
N 140 -510 140 -390 {
lab=GNDI}
N 120 -510 140 -510 {
lab=GNDI}
N -330 -510 -330 -490 {
lab=#net10}
N -440 -300 -330 -300 {
lab=#net6}
N -210 -420 -210 -340 {
lab=#net8}
N -330 -430 -330 -350 {
lab=#net6}
N -310 -720 -130 -720 {
lab=VDDI}
N 50 -300 60 -300 {
lab=#net11}
N -130 -720 -0 -720 {
lab=VDDI}
N 100 -300 140 -300 {
lab=GNDI}
N 40 -300 50 -300 {
lab=#net11}
N -280 -120 90 -120 {
lab=GNDI}
N 90 -120 190 -120 {
lab=GNDI}
N -170 -510 -170 -300 {
lab=GNDI}
N -210 -570 -170 -570 {
lab=GNDI}
N -170 -570 -170 -510 {
lab=GNDI}
N 30 -300 40 -300 {
lab=#net11}
N 100 -420 100 -330 {
lab=#net12}
N -60 -300 -30 -300 {
lab=#net13}
N -120 -300 -60 -300 {
lab=#net13}
N -30 -390 -30 -300 {
lab=#net13}
N -30 -600 -30 -570 {
lab=#net9}
N -30 -600 -20 -600 {
lab=#net9}
N -210 -150 -210 -120 {
lab=GNDI}
N -170 -170 -170 -120 {
lab=GNDI}
N 100 -150 100 -120 {
lab=GNDI}
N 140 -390 140 -120 {
lab=GNDI}
N -560 -290 -560 -120 {
lab=GNDI}
N -490 -380 -490 -330 {
lab=GNDI}
N -560 -330 -490 -330 {
lab=GNDI}
N -590 -380 -590 -330 {
lab=GNDI}
N -590 -330 -560 -330 {
lab=GNDI}
N -560 -720 -560 -590 {
lab=VDDI}
N -30 -510 -30 -490 {
lab=#net14}
N -30 -430 -30 -390 {
lab=#net13}
N -120 -340 -120 -300 {
lab=#net13}
N -400 -340 -400 -300 {
lab=#net6}
N -400 -400 -120 -400 {
lab=Vref}
N -520 -440 -400 -440 {
lab=Vref}
N -400 -440 -400 -400 {
lab=Vref}
N -100 -600 -70 -600 {
lab=#net13}
N -70 -600 -70 -300 {
lab=#net13}
N 220 -600 220 -520 {
lab=#net15}
N 210 -600 220 -600 {
lab=#net15}
N 220 -460 220 -120 {
lab=GNDI}
N 190 -120 220 -120 {
lab=GNDI}
N 0 -720 220 -720 {
lab=VDDI}
N -560 -720 -310 -720 {
lab=VDDI}
N 100 -270 100 -210 {
lab=#net16}
N -210 -270 -210 -210 {
lab=#net17}
N -560 -330 -560 -290 {
lab=GNDI}
N -560 -410 -560 -330 {
lab=GNDI}
N -210 -540 -210 -480 {
lab=#net18}
N 100 -540 100 -480 {
lab=#net19}
C {devices/code.sym} 310 -885 0 0 {name=TT_MODELS only_toplevel=false
format="tcleval(@value )" value=".lib \\\\$::SKYWATER_MODELS\\\\/sky130.lib.spice tt
* .include \\\\$::PDKPATH\\\\/libs.ref/sky130_fd_sc_hvl/spice/sky130_fd_sc_hvl.spice
"}
C {devices/code_shown.sym} 310 -735 0 0 {name=s2 only_toplevel=false value="
.subckt sky130_fd_pr__rf_nfet_01v8_bM04W5p00L0p15_ext DRAIN GATE SOURCE SUBSTRATE
X0 DRAIN GATE SOURCE SUBSTRATE sky130_fd_pr__nfet_01v8 ad=2.828e+12p pd=2.132e+07u as=4.242e+12p ps=3.198e+07u w=5.05e+06u l=150000u
X1 SOURCE SUBSTRATE SUBSTRATE SUBSTRATE sky130_fd_pr__nfet_01v8 ad=0p pd=0u as=3.03e+12p ps=2.14e+07u w=5.05e+06u l=150000u
X2 SOURCE GATE DRAIN SUBSTRATE sky130_fd_pr__nfet_01v8 ad=0p pd=0u as=0p ps=0u w=5.05e+06u l=150000u
X3 DRAIN GATE SOURCE SUBSTRATE sky130_fd_pr__nfet_01v8 ad=0p pd=0u as=0p ps=0u w=5.05e+06u l=150000u
X4 SUBSTRATE SUBSTRATE SOURCE SUBSTRATE sky130_fd_pr__nfet_01v8 ad=0p pd=0u as=0p ps=0u w=5.05e+06u l=150000u
X5 SOURCE GATE DRAIN SUBSTRATE sky130_fd_pr__nfet_01v8 ad=0p pd=0u as=0p ps=0u w=5.05e+06u l=150000u
.ends
.sp dec 1000 1e9 10e9 1
* .ac dec 1000 0.01e9 100e9 
.control
run
display
let z11=50*(1+s_1_1)/(1-s_1_1)
let z22=50*(1+s_2_2)/(1-s_2_2)
let S11 = S_1_1
let S21 = S_2_1
let S12 = S_1_2
let S22 = S_2_2

* Stability
let delta=S11*S22-S12*S21
let K    = (1-mag(S11)^2-mag(S22)^2+mag(delta)^2)/(2*mag(S12*S21))
let mu   = (1-mag(S11)^2)/(mag(S22-conj(S11)*delta)+mag(S21*S12))
* plot real(z11) real(z22)
* plot imag(z11) imag(z22)
plot db(S_1_1) db(S_2_2) db(S_2_1)

plot db(K)
plot mag(delta)
plot db(mu)
plot NF NFmin
.endc
"}
C {devices/vsource.sym} -800 -830 0 0 {name=V1 value=1.8}
C {devices/gnd.sym} -800 -640 0 0 {name=l3 lab=GND}
C {devices/vsource.sym} -815 -230 0 0 {name=V2 value="dc 0.9 ac 1 portnum 1 z0 50"
}
C {devices/isource.sym} -560 -560 0 0 {name=I0 value=1.1m}
C {devices/nmos4.sym} -540 -440 0 1 {name=XM1 model=sky130_fd_pr__rf_nfet_01v8_bM04W5p00L0p15_ext w=5.05u l=0.3u m=1}
C {sky130_fd_pr/cap_mim_m3_1.sym} -490 -410 0 0 {name=C7 model=cap_mim_m3_1 W=20 L=30 MF=1 spiceprefix=X}
C {devices/ind.sym} -710 -950 0 0 {name=Ldeg5
m=1
value=2n
footprint=1206
device=inductor}
C {devices/res.sym} -710 -1030 2 0 {name=R3
value=3
footprint=1206
device=resistor
m=1}
C {devices/ind.sym} -710 -670 0 0 {name=Ldeg6
m=1
value=2n
footprint=1206
device=inductor}
C {devices/res.sym} -710 -750 2 0 {name=R4
value=3
footprint=1206
device=resistor
m=1}
C {devices/nmos4.sym} -230 -300 0 0 {name=XM12 model=sky130_fd_pr__rf_nfet_01v8_bM04W5p00L0p15_ext w=5.05u l=0.3u m=3}
C {devices/capa.sym} -130 -600 1 0 {name=C17
m=1
value=1p
footprint=1206
device="ceramic capacitor"}
C {devices/ind.sym} -280 -665 0 0 {name=L2
m=1
value=2.5n
footprint=1206
device=inductor}
C {sky130_fd_pr/cap_mim_m3_1.sym} -500 -820 0 0 {name=C3 model=cap_mim_m3_1 W=40 L=60 MF=8 spiceprefix=X}
C {devices/gnd.sym} -560 -120 0 0 {name=l4 lab=GNDI}
C {devices/capa.sym} -630 -710 0 0 {name=C8
m=1
value=200f

footprint=1206
device="ceramic capacitor"}
C {devices/capa.sym} -630 -1000 0 0 {name=C19
m=1
value=200f

footprint=1206
device="ceramic capacitor"}
C {devices/lab_wire.sym} -800 -1060 0 0 {name=l14 sig_type=std_logic lab=VDD}
C {devices/lab_wire.sym} -580 -910 0 0 {name=l1 sig_type=std_logic lab=VDDI}
C {devices/gnd.sym} -580 -790 0 0 {name=l2 lab=GNDI}
C {devices/res.sym} -500 -880 2 0 {name=R2
value=5
footprint=1206
device=resistor
m=1}
C {devices/gnd.sym} -815 -200 0 0 {name=l17 lab=GND}
C {devices/res.sym} -400 -370 2 0 {name=R5
value=3k
footprint=1206
device=resistor
m=1}
C {devices/capa.sym} -775 -300 1 0 {name=C1
m=1
value=1n
footprint=1206
device="ceramic capacitor"}
C {devices/capa.sym} -210 -660 2 1 {name=C6
m=1
value=150f
footprint=1206
device="ceramic capacitor"}
C {devices/res.sym} -330 -540 2 0 {name=R6
value=3k

footprint=1206
device=resistor
m=1}
C {devices/nmos4.sym} -230 -570 0 0 {name=XM3 model=sky130_fd_pr__rf_nfet_01v8_bM04W5p00L0p15_ext w=5.05u l=0.3u m=6}
C {devices/lab_wire.sym} -310 -720 0 0 {name=l12 sig_type=std_logic lab=VDDI}
C {devices/ind.sym} -690 -300 1 0 {name=Ldeg1
m=1
value=3n
footprint=1206
device=inductor}
C {devices/ind.sym} -210 -450 0 0 {name=L1
m=1
value=1n
footprint=1206
device=inductor}
C {devices/ind.sym} -300 -300 1 0 {name=Ldeg2
m=1
value=1.5n
footprint=1206
device=inductor}
C {devices/vsource.sym} 220 -490 0 0 {name=V3 value="DC 0.9 AC 0 portnum 2 z0 50"}
C {devices/ind.sym} 100 -180 0 0 {name=L5
m=1
value=0.1n
footprint=1206
device=inductor}
C {devices/ind.sym} 0 -300 1 0 {name=L7
m=1
value=1.4n
footprint=1206
device=inductor}
C {devices/res.sym} -120 -370 2 0 {name=R1
value=3k
footprint=1206
device=resistor
m=1}
C {devices/nmos4.sym} 80 -300 0 0 {name=XM5 model=sky130_fd_pr__rf_nfet_01v8_bM04W5p00L0p15_ext w=5.05u l=0.3u m=3}
C {sky130_fd_pr/cap_mim_m3_1.sym} -400 -820 0 0 {name=C4 model=cap_mim_m3_1 W=40 L=60 MF=8 spiceprefix=X}
C {devices/res.sym} -400 -880 2 0 {name=R8
value=5
footprint=1206
device=resistor
m=1}
C {devices/ind.sym} 30 -665 0 0 {name=L3
m=1
value=2.5n
footprint=1206
device=inductor}
C {devices/capa.sym} 100 -660 2 1 {name=C2
m=1
value=200f
footprint=1206
device="ceramic capacitor"}
C {devices/res.sym} -30 -540 2 0 {name=R9
value=3k

footprint=1206
device=resistor
m=1}
C {devices/nmos4.sym} 80 -570 0 0 {name=XM8 model=sky130_fd_pr__rf_nfet_01v8_bM04W5p00L0p15_ext w=5.05u l=0.3u m=6}
C {devices/lab_wire.sym} 0 -720 0 0 {name=l5 sig_type=std_logic lab=VDDI}
C {devices/ind.sym} 100 -450 0 0 {name=L6
m=1
value=1n
footprint=1206
device=inductor}
C {devices/capa.sym} 180 -600 1 0 {name=C5
m=1
value=200f
footprint=1206
device="ceramic capacitor"}
C {devices/capa.sym} -30 -460 2 1 {name=C9
m=1
value=1p
footprint=1206
device="ceramic capacitor"}
C {devices/capa.sym} -330 -460 2 1 {name=C10
m=1
value=1p
footprint=1206
device="ceramic capacitor"}
C {devices/ind.sym} -210 -180 0 0 {name=L4
m=1
value=0.1n
footprint=1206
device=inductor}
C {devices/lab_wire.sym} -420 -440 0 0 {name=l6 sig_type=std_logic lab=Vref}
